...
| フィールド | 説明 |
|-----------|------|
| `name` | Podcast 名（表示・ディレクトリ名に使用） |
| `id` | Podcast Index の Feed ID |
| `language` | 元の言語コード。`ja`（デフォルト）または `en` など |

**language の動作:**
- `ja` または未指定: 日本語 Podcast として処理（翻訳なし）
- `en` など: 元の言語で書き起こし・要約を生成し、日本語に翻訳。原文も保持
